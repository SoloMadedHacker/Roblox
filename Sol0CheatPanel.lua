...
OrionLib:MakeNotification({
    ...
    Content = "Executed On: " .. executorName,
    Image = "rbxassetid://4483345998",
    Time = 5
})
...
-- Function to handle player notifications
local function NotifyPlayerAction(action, player)
    OrionLib:MakeNotification({
        Name = "Player Info",
        Content = player.Name .. " " .. action,
        Image = "rbxassetid://4483345998",
        Time = 2  -- Notification lasts for 2 seconds
    })
end
...
local function TrackPlayers()
    ...
    Players.PlayerAdded:Connect(function(player)
        if toggleOn then
            NotifyPlayerAction("had joined!", player)
            
            player.CharacterAdded:Connect(function(character)
                -- Player respawned
                NotifyPlayerAction("had spawned!", player)
                -- Player died
                character:WaitForChild("Humanoid").Died:Connect(function()
                    NotifyPlayerAction("had died!", player)
                end)
            end)
        end
    end)
    ...
    Players.PlayerRemoving:Connect(function(player)
        if toggleOn then
            NotifyPlayerAction("had left!", player)
        end
    end)

    -- Track local player as well (for respawn and death)
    LocalPlayer.CharacterAdded:Connect(function(character)
        if toggleOn then
            NotifyPlayerAction("had spawned!", LocalPlayer)
            character:WaitForChild("Humanoid").Died:Connect(function()
                NotifyPlayerAction("had died!", LocalPlayer)
            end)
        end
    end)
end

-- Add toggle for InfoGetterPlayer
Info:AddToggle({
    Name = "InfoGetterPlayer",
    Default = false,
...
local AdultScripts = Window:MakeTab({
	Name = "AdultScripts",
	Icon = "rbxassetid://4483345998",
	PremiumOnly = false
})

AdultScripts:AddButton({
    Name = "Jerk Co*k (Includes c*m)",
    Callback = function()
        loadstring(game:HttpGet("https://pastebin.com/raw/Uzc8aWSw"))()
    end
})
